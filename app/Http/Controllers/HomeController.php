...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $tableDate = DB::select('SELECT (DATE_FORMAT(created_at,"%m-%d-%Y")) as dateTaken FROM `test_answers`WHERE question_id in (1,2,3,4) AND user_id= '.$id.'  GROUP BY (created_at) ORDER BY (created_at) ASC LIMIT 5');
       
        $tableSection12 = DB::select('SELECT SUM(correct) as result, test_id as attempt FROM `test_answers` WHERE question_id in (1,2,3,4) AND user_id= '.$id.' GROUP BY ( test_id) ASC LIMIT 5 ');

        $tableSection13 = DB::select('SELECT  SUM(correct) as result, test_id as attempt FROM `test_answers` WHERE question_id in (5,6,7,8) AND user_id= '.$id.' GROUP BY ( test_id) ASC LIMIT 5  ');

        $tableSection14 = DB::select('SELECT  SUM(correct) as result, test_id as attempt FROM `test_answers`WHERE question_id in (9,10,11,12) AND user_id= '.$id.' GROUP BY ( test_id) ASC LIMIT 5  ');
