...
class ResultGraphController extends Controller
{
    public function SurveyResultGraph(){
        ...
        //$graphSection1 = DB::select('SELECT COUNT(id) as result FROM `test_answers`WHERE question_id in (1,2,3,4) AND correct = 1 GROUP BY  (created_at) ORDER BY  (created_at) ');
        //dd($graphSection1);
       // $graphSection2 = DB::select('SELECT COUNT(id) as result, test_id as attempt FROM `test_answers`WHERE  question_id in (5,6,7,8) AND correct = 1 AND user_id= '.$id.' GROUP BY (test_id) ORDER BY (test_id) ');
        //$graphSection3 = DB::select('SELECT COUNT(id) as result, test_id as attempt FROM `test_answers`WHERE   question_id in (9,10,11,12) AND correct = 1 AND user_id= '.$id.' GROUP BY  (test_id) ORDER BY  (test_id) ');
