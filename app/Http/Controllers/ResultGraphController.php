...
class ResultGraphController extends Controller
{
    public function SurveyResultGraph(){
        $id = Auth::id();
        $graphTotal = Test::select('result')->where('user_id', auth()->id())
        ->get();   
        //graphSection1 is query for self care section
        $graphSection1 = DB::select('SELECT SUM(correct) as result FROM
         `test_answers` WHERE question_id in (1,2,3,4) AND user_id= '.$id.' 
         GROUP BY ( test_id) ');
        //graphSection2 is query for health awareness section
        $graphSection2 =DB::select('SELECT SUM(correct) as result FROM 
        `test_answers` WHERE question_id in (5,6,7,8) AND user_id= '.$id.' 
        GROUP BY ( test_id)  ');
        //graphSection3 is query for communication section
        $graphSection3 = DB::select('SELECT SUM(correct) as result FROM
         `test_answers` WHERE question_id in (9,10,11,12) AND user_id= '.$id.' 
         GROUP BY ( test_id)  ');
        ...
        return \View::make('ResultGraph')->with('graphJsonTotal', $graphJsonTotal)
        ->with('graphJsonSection1', $graphJsonSection1)
        ->with('graphDateJson', $graphDateJson)->
        with('graphJsonSection2', $graphJsonSection2)
        ->with('graphJsonSection3', $graphJsonSection3);
    }
}